...
const faqItems = [
  {
    ...
    content:
      `Наша платформа — это инструмент для всех, кто занимается коммерцией. Она создана для селлеров на любых маркетплейсах (Wildberries, Ozon, Яндекс Маркет и др.), позволяя быстро создать упаковку товара. В режиме «${KARTO_CREATIVE_MODE.title}» (${KARTO_CREATIVE_MODE.tagline.toLowerCase()}) сервис также полезен дизайнерам и маркетологам, которым нужны уникальные изображения и видео без затрат на студийную съёмку.`,
  },
  {
  ...
  {
    title: `В чем разница между «${KARTO_FLOW_MODE.title}» и «${KARTO_CREATIVE_MODE.title}»?`,
    content:
      `${KARTO_FLOW_MODE.title} — ${KARTO_FLOW_MODE.tagline.toLowerCase()}: 1 250 кредитов на запуск (до 10 фото 4K и 2 видео, или другой микс), SEO-описание и анализ цен. ${KARTO_CREATIVE_MODE.title} — ${KARTO_CREATIVE_MODE.tagline.toLowerCase()}: ${KARTO_CREATIVE_VALUE_PROP.toLowerCase()} из одного баланса кредитов по вашим промптам. Пакеты кредитов покупаются отдельно и не заменяют Поток.`,
  },
  {
    title: "Как долго действуют приобретенные пакеты?",
    content:
      `Купленные «Потоки» и пакеты кредитов в «${KARTO_CREATIVE_MODE.title}» активны 30 дней с момента оплаты (при новой покупке того же типа период продлевается). Неиспользованные остатки по истечении периода аннулируются. Остаток кредитов внутри завершённого Потока сгорает.`,
  },
  {
    title: "Сколько вариантов дизайна я получу в одном «Потоке»?",
    content:
      "В одном Потоке — 1 250 кредитов: до 10 фото 4K и 2 видео, или другой микс. Текст описания и анализ цены не списывают кредиты. Неиспользованные кредиты сгорают при завершении Потока.",
  },
  {
  ...
  },
  {
    ...
    content:
      `Конечно. Хотя мы делаем акцент на маркетплейсах, возможности системы гораздо шире. В «${KARTO_CREATIVE_MODE.title}» вы можете создавать рекламные баннеры, креативы для постов, короткие видеоролики и обложки. Результаты можно скачивать и использовать на любых площадках.`,
  },
]
