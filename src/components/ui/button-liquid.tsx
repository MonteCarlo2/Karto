...
const GradientSvg: React.FC<GradientSvgProps> = ({
  ...
  const gradientTransformList = svgOrder.map(
    (svgKey) => svgStates[svgKey].gradientTransform
  )

  const variants = {
    hovered: {
      gradientTransform: gradientTransformList,
    ...
    >
      ...
      <defs>
        <motion.radialGradient
          ...
          gradientTransform={gradientTransformList[0]}
...
/** Палитра под KARTO: тёмно-зелёный и салатовый */
export const KARTO_LIQUID_COLORS: Colors = {
  color1: "#F5F5F0",
  color2: "#1F4E3D",
  color3: "#2E5A43",
  color4: "#FCFCFE",
...
export function LiquidCtaButton({
  ...
  return (
    <div className={`flex justify-center ${className}`.trim()}>
      <Link
        href={href}
        className="relative inline-block h-[2.7em] w-full min-w-[14rem] max-w-sm mx-auto group dark:bg-black bg-white dark:border-white border-black border-2 rounded-xl sm:rounded-2xl overflow-hidden"
      ...
      >
        <div className="absolute w-[112.81%] h-[128.57%] top-[8.57%] left-1/2 -translate-x-1/2 filter blur-[19px] opacity-70 pointer-events-none">
          <span className="absolute inset-0 rounded-xl sm:rounded-2xl bg-[#d9d9d9] filter blur-[6.5px]" />
          <div className="relative w-full h-full overflow-hidden rounded-xl sm:rounded-2xl">
            <Liquid isHovered={isHovered} colors={KARTO_LIQUID_COLORS} />
          </div>
        </div>
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[40%] w-[92.23%] h-[112.85%] rounded-xl sm:rounded-2xl bg-[#0a1f18] filter blur-[7.3px] pointer-events-none" />
        <div className="relative w-full h-full overflow-hidden rounded-xl sm:rounded-2xl pointer-events-none">
          <span className="absolute inset-0 rounded-xl sm:rounded-2xl bg-[#d9d9d9]" />
          <span className="absolute inset-0 rounded-xl sm:rounded-2xl bg-neutral-900" />
          <Liquid isHovered={isHovered} colors={KARTO_LIQUID_COLORS} />
          {[1, 2, 3, 4, 5].map((i) => (
            <span
              key={i}
              className={`absolute inset-0 rounded-xl sm:rounded-2xl border-solid border-[3px] border-white/20 mix-blend-overlay filter ${i <= 2 ? "blur-[3px]" : i === 3 ? "blur-[5px]" : "blur-[4px]"}`}
            />
          ))}
          <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[40%] w-[70.8%] h-[42.85%] rounded-lg filter blur-[15px] bg-[#1F4E3D]/60" />
        </div>
        <span className="absolute inset-0 flex items-center justify-center rounded-xl sm:rounded-2xl text-lg font-semibold tracking-wide text-white group-hover:text-[#a7f3d0] transition-colors pointer-events-none">
          {children}
        </span>
      </Link>
    </div>
