/**
 * Сервис для генерации описаний товаров через OpenRouter API (chat/completions).
 */

// Используем только ключ из окружения (без fallback), чтобы не маскировать проблемы конфигурации
import {
  getNormalizedOpenRouterApiKey,
  getOpenRouterRequestHeaders,
} from "@/lib/openrouter-headers";
import {
  buildOpenRouterModelChain,
  isOpenRouterModelRoutingError,
  resolveOpenRouterTextPrimary,
} from "@/lib/openrouter-default-models";
import { openRouterFetch } from "@/lib/openrouter-fetch";
import { lightSanitizeDescriptionStream } from "@/lib/utils/marketplace-formatter";
import { ensureSellingHighlights } from "@/lib/utils/description-markup";

const OPENROUTER_API_URL = "https://openrouter.ai/api/v1/chat/completions";

/** Основная модель для описаний (OpenRouter id). */
function getPrimaryDescriptionModel(): string {
  return resolveOpenRouterTextPrimary();
}

function sleep(ms: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

/**
 * Удаляем markdown-заголовки (#, ##, ###...), которые иногда возвращает модель
 * несмотря на инструкции в промпте.
 */
function sanitizeDescriptionOutput(text: string, markHighlights = false): string {
  if (markHighlights) {
    // Для SEO-описаний сохраняем **жирный** и ⟦триггеры⟧
    const cleaned = text
      .replace(/^\s{0,3}#{1,6}\s+/gm, "")
      .replace(/`([^`]+)`/g, "$1")
      .replace(/\r\n/g, "\n")
      .replace(/\n{3,}/g, "\n\n")
      .trim();
    return ensureSellingHighlights(cleaned);
  }
  return lightSanitizeDescriptionStream(text);
}

async function readDescriptionFromOpenRouterResponse(
  response: Response,
  style: number,
  markHighlights = false
): Promise<string> {
  const data = await response.json();
  let content = data?.choices?.[0]?.message?.content;
  if (Array.isArray(content)) {
    content = content
      .map((part: { text?: string }) => (typeof part?.text === "string" ? part.text : ""))
      .join("");
  }
  const description = typeof content === "string" ? content.trim() : "";
  if (!description || description.length < 80) {
    throw new Error("OpenRouter вернул пустой/слишком короткий ответ");
  }
  return sanitizeDescriptionOutput(description, markHighlights);
}

export type DescriptionTextLength = "shorter" | "medium" | "longer";

export type GenerateDescriptionOptions = {
  textLength?: DescriptionTextLength;
  /** Обернуть ключевые продающие фразы в маркеры ⟦…⟧ для UI-подсветки */
  markHighlights?: boolean;
};

const LENGTH_REQUIREMENTS: Record<DescriptionTextLength, string> = {
  shorter:
    "СТРОГО 120–220 слов. Компактный текст: суть и главные выгоды без воды. Не раздувай абзацы. Это режим «Короче».",
  medium:
    "200–500 слов. Автоматический стандартный объём для карточки маркетплейса (режим «Авто»).",
  longer:
    "СТРОГО 450–700 слов. Развёрнутое описание: сценарии использования, детали и выгоды, без воды и клише. Это режим «Длиннее».",
};

/**
 * Генерация описания товара через OpenRouter
 * Полностью сохраняет функционал и промпты из Replicate версии
 */
export async function generateProductDescription(
  productName: string,
  userPreferences: string = "",
  selectedBlocks: string[] = [],
  photoUrl?: string,
  style: 1 | 2 | 3 | 4 = 1,
  wantsStickers: boolean = false,
  baseDescription?: string,
  options: GenerateDescriptionOptions = {}
): Promise<string> {
  const textLength: DescriptionTextLength = options.textLength ?? "medium";
  const markHighlights = Boolean(options.markHighlights);
  const styleNames = { 1: "Официальный", 2: "Продающий", 3: "Структурированный", 4: "Сбалансированный" };
  const primaryModel = getPrimaryDescriptionModel();
  console.log(
    `🔄 [СТИЛЬ ${style}] Генерируем описание через OpenRouter, модель ${primaryModel} (${styleNames[style]})...`
  );
  console.log(`🔄 [СТИЛЬ ${style}] Товар: "${productName}"`);
  
  // Определяем стиль описания (ТОЧНО ТАК ЖЕ, как в Replicate версии)
  const stylePrompts = {
    1: "Официальный/технический: фокус на фактах и характеристиках, но без сухости. Естественный язык, как будто эксперт рассказывает о товаре.",
    2: "Продающий/эмоциональный: покажи выгоды и преимущества, но без инфоцыганства. Честно и убедительно, как хороший продавец в магазине.",
    3: "Структурированный: четкая организация, списки и акценты, но не шаблонно. Как профессиональный обзор от реального пользователя.",
    4: "Сбалансированный: комбинация всех элементов. Естественный рассказ о товаре, как будто друг рекомендует что-то хорошее.",
  };
  
  const styleDescription = stylePrompts[style];
  
  // Формируем список выбранных блоков (ТОЧНО ТАК ЖЕ)
  const blocksText = selectedBlocks.length > 0
    ? `\nОбязательно включи в описание:\n${selectedBlocks.map(b => `- ${b}`).join("\n")}`
    : "";

  const highlightsInstruction = markHighlights
    ? `
ПОДСВЕТКА ПРОДАЮЩИХ ТРИГГЕРОВ (обязательно в КАЖДОМ описании):
- Отметь ровно 5–7 коротких фраз маркерами ⟦фраза⟧ — конкретные выгоды, свойства, материалы, цифры.
- Это «крючки» для покупателя: не общие слова вроде «качество»/«удобно», а ощутимая ценность («⟦лёгкий авиационный алюминий⟧», «⟦зарядка за 30 минут⟧»).
- Фраза внутри маркеров: 2–6 слов. Не выделяй целые предложения и не заголовки.
- Не чаще: максимум одна подсветка на 1–2 предложения. Запрещено подсвечивать каждое второе слово.
- Маркеры ⟦ ⟧ обязательны — без них описание считается незавершённым.`
    : "";

  const preferencesBlock = userPreferences.trim()
    ? `
ПОЖЕЛАНИЯ ПОЛЬЗОВАТЕЛЯ — МАКСИМАЛЬНЫЙ ПРИОРИТЕТ (важнее стиля ниже):
"""
${userPreferences.trim()}
"""
Выполни каждое пожелание явно. Если пожелание конфликтует со стилем — побеждают пожелания.`
    : "";
  
  // Системный промпт (ТОЧНО ТАК ЖЕ, как в Replicate версии)
  const systemPrompt = `Ты профессиональный копирайтер для маркетплейсов. Твоя задача - писать описания товаров так, как пишет опытный копирайтер-человек.

КРИТИЧЕСКИ ВАЖНО:
- Пиши только на русском языке (кириллица). Не вставляй китайские, японские, корейские иероглифы и не смешивай языки.
- Пиши как человек, а не как шаблон. Используй естественный язык, живые формулировки.
- Будь честным и убедительным, но НЕ инфоцыгань. Никаких "РЕВОЛЮЦИОННЫХ ПРОРЫВОВ", "ЕДИНСТВЕННЫХ В СВОЕМ РОДЕ", "МЕНЯЕТ ЖИЗНЬ".
- Не используй громкие слоганы и клише типа "качество премиум", "невероятная выгода", "лучшее предложение".
- Не пиши так, будто продаёшь плохой товар. Будь уверенным, но не навязчивым.
- Фокус на реальных преимуществах и характеристиках, поданных интересно.
- Используй конкретику, примеры использования, реальные сценарии.
- Структура должна быть естественной, не шаблонной "характеристики-преимущества-комплектация".
- Пиши так, чтобы покупатель захотел купить, но не чувствовал себя обманутым.
- Не превращай описание в сухой каталог: избегай десятка строк подряд, каждая с дефисом и одной короткой фразой. Так выглядит шаблон маркетплейса, а не живой текст.
- Основа текста — связные абзацы (несколько предложений подряд), как в хорошей статье или отзыве. Цифры и факты вплетай в предложения ("оправа из лёгкого металла, ширина 140 мм…"), а не выносай каждую в отдельную строку с дефисом.
- Списки с дефисом используй умеренно: например один короткий блок в конце ("Для кого подойдёт:") или 3–6 пунктов там, где перечисление действительно уместно. Не делай весь текст из списков.

Стиль: профессиональный, но живой. Как хороший продавец, который знает товар и честно о нём рассказывает.`;

  // Формируем промпт для пользователя (ТОЧНО ТАК ЖЕ)
  const baseTextBlock = baseDescription
    ? `Исходный текст (перепиши его, строго учитывая все правки):\n\"\"\"\n${baseDescription}\n\"\"\"`
    : "";

  const userPrompt = `Напиши описание товара для маркетплейса (Ozon, Wildberries).

Товар: ${productName}
${preferencesBlock}
${blocksText}
${baseTextBlock ? `\n${baseTextBlock}\n` : ""}

Стиль: ${styleDescription}

КРИТИЧЕСКИ ВАЖНО ПО ФОРМАТИРОВАНИЮ И КОМПОЗИЦИИ:
- НИКОГДА не используй markdown-хэштеги (# или ##) в тексте
- Нужен СМЕШАННЫЙ вид: больше обычного текста (абзацы по 2–5 предложений), меньше «простыни» из дефисов. Хороший эталон: вступление связным текстом, затем 2–4 подзаголовка с двоеточием, под каждым — цельный абзац с мыслью, а не набор строк с дефисом; при необходимости один компактный список в конце.
- Запрещённый приём: блок «Характеристики:» из 8–15 строк, где КАЖДАЯ строка начинается с «-» и на одной строке только название параметра, а значение на следующей. Вместо этого — 1–2 абзаца, где параметры перечислены внутри предложений (через запятые, «и», скобки), либо один короткий список из 3–5 самых важных пунктов.
- Для подзаголовков секций используй строку с двоеточием в конце (например: «Дизайн и комфорт:»), с новой строки сразу связный абзац, без разбиения абзаца на десяток пунктов с дефисом.
- Списки с дефисом (-) или маркером (•) — только там, где уместно перечисление (аудитория, комплектация, 3–5 ключевых плюсов). Не более одного-двух таких блоков на всё описание, каждый не длиннее 6 пунктов, если только пользователь явно не просит иначе.
- НЕ используй символы → и другие нестандартные буллеты — в Ozon они могут отображаться плохо
- Между абзацами оставляй одну пустую строку
- Переносы строк — естественные, не дроби текст на одно слово/фразу в строку

${wantsStickers ? `ВАЖНО: Добавь простые эмодзи (стикеры) в описание где это уместно. Используй: ✓, ★, 🎁, ❤️, 🔥, ✨\n- Используй их умеренно, но обязательно включи несколько в текст\n- Размещай их там, где это улучшает восприятие (например, ✓ перед преимуществами, ★ для акцентов, 🎁 для подарков)` : `- БЕЗ эмодзи и стикеров (если не указано иное в пожеланиях)`}

Требования:
- Объём: ${LENGTH_REQUIREMENTS[textLength]}
- Естественный язык, как от копирайтера-человека
- Без шаблонов и клише
- Без громких слоганов
- Честно и убедительно
- Только русский язык, без иероглифов и без фрагментов на других языках
- Живой, интересный текст, не сухой; минимум половины объёма — связные абзацы, не пункты с дефисом
${highlightsInstruction}

Напиши описание:`;

  try {
    if (!getNormalizedOpenRouterApiKey()) {
      throw new Error("OPENROUTER_API_KEY не настроен или пустой после нормализации");
    }
    console.log(`🔄 [СТИЛЬ ${style}] Запускаем запрос к OpenRouter...`);
    console.log(`🔄 [СТИЛЬ ${style}] Длина промпта: ${userPrompt.length} символов`);
    
    const messages = [
      { role: "system" as const, content: systemPrompt },
      { role: "user" as const, content: userPrompt },
    ];

    const requestBodyBase = {
      messages,
      stream: false,
      temperature: 0.7,
      max_tokens: 2000,
    };

    const modelChain = buildOpenRouterModelChain(primaryModel);

    // Делаем запрос с retry логикой
    let attempts = 0;
    const maxAttempts = 3;
    let modelIndex = 0;
    while (attempts < maxAttempts) {
      try {
        const activeModel = modelChain[modelIndex] ?? modelChain[0];
        console.log(
          `🔄 [СТИЛЬ ${style}] Запрос к OpenRouter (попытка ${attempts + 1}/${maxAttempts}, модель ${activeModel})...`
        );

        const response = await openRouterFetch(OPENROUTER_API_URL, {
          method: "POST",
          headers: getOpenRouterRequestHeaders("KARTO - Product Description Generator"),
          body: JSON.stringify({ ...requestBodyBase, model: activeModel }),
        });

        if (!response.ok) {
          const errorText = await response.text();
          const lastStatus = response.status;

          console.error(`❌ [СТИЛЬ ${style}] OpenRouter API error: ${lastStatus} - ${errorText}`);

          if (isOpenRouterModelRoutingError(lastStatus, errorText) && modelIndex < modelChain.length - 1) {
            modelIndex += 1;
            console.warn(
              `⚠️ [СТИЛЬ ${style}] Модель ${activeModel} недоступна, пробуем ${modelChain[modelIndex]}`
            );
            continue;
          }

          if (lastStatus === 429) {
            attempts++;
            const retryAfter = response.headers.get("retry-after");
            const delay = retryAfter ? parseInt(retryAfter, 10) * 1000 : 10000;
            console.log(`⏳ [СТИЛЬ ${style}] Rate limit (429), ждем ${delay / 1000}s перед повтором...`);
            await sleep(delay);
            continue;
          }

          throw new Error(`OpenRouter API error: ${lastStatus} - ${errorText}`);
        }

        const description = await readDescriptionFromOpenRouterResponse(
          response,
          style,
          markHighlights
        );
        console.log(
          `✅ [СТИЛЬ ${style}] Описание сгенерировано (${modelChain[modelIndex] ?? primaryModel}, ${description.length} символов)`
        );
        return description;
      } catch (requestError: any) {
        attempts++;
        const isRateLimit = String(requestError?.message || "").includes("429");
        const isLastAttempt = attempts >= maxAttempts;
        if (!isLastAttempt && isRateLimit) {
          const delay = 5000 * attempts;
          console.log(`⏳ [СТИЛЬ ${style}] Повтор после rate limit через ${delay/1000}с...`);
          await sleep(delay);
          continue;
        }
        if (!isLastAttempt) {
          const delay = 1500 * attempts;
          console.log(`⏳ [СТИЛЬ ${style}] Повтор через ${delay/1000}с из-за ошибки...`);
          await sleep(delay);
          continue;
        }
        throw requestError;
      }
    }

    throw new Error(`[СТИЛЬ ${style}] Не удалось получить ответ от OpenRouter после ${maxAttempts} попыток`);

  } catch (error: any) {
    const errorMessage = String(error?.message || error || "");
    console.error(`❌ [СТИЛЬ ${style}] Ошибка генерации через OpenRouter:`, errorMessage);
    console.error(`❌ [СТИЛЬ ${style}] Stack trace:`, error?.stack);
    throw error;
  }
}
