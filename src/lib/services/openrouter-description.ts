...
function getPrimaryDescriptionModel(): string {
  return (process.env.OPENROUTER_DESCRIPTION_MODEL || "").trim() || "anthropic/claude-sonnet-4.5";
}

/**
 * При 403 «Terms of Service» у Anthropic через OpenRouter — вторая попытка с этой моделью.
...
function getFallbackDescriptionModel(primary: string): string | null {
  ...
  const fb = (raw || "").trim() || "openai/gpt-4o-mini";
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (lastStatus === 403 && /Terms Of Service|violation/i.test(errorText)) {
            console.error(
              `[СТИЛЬ ${style}] 403 ToS: часто блокирует anthropic/* на OpenRouter; задайте OPENROUTER_DESCRIPTION_MODEL или проверьте кабинет OpenRouter.`
            );
          }
