...
## 1. Timeweb: Nest/Node (ваши текущие настройки)

**В корне репозитория нет `Dockerfile` намеренно.** Если он лежит в корне, Timeweb запускает сборку через Docker, вызывает `apt-get install curl` к `deb.debian.org`, и деплой падает (DNS/IPv6 на их сборочной сети) — даже при пустом поле «Зависимости».

**Правильные настройки (как у вас на скриншоте):**

| Поле | Значение |
|------|----------|
| Окружение | Node.js **24** |
| Фреймворк | Nest *(название не важно — это Next.js)* |
| Сборка | `npm run build` |
| **Зависимости** | **пусто** |
| Запуск | **`node start.js`** (из корня репо) |
| Health check | **`/api/health`** или **`/health`** |
| Env | не трогать |

После пуша: **«Сбросить кэш и пересобрать»**.

Подсказка TimewebGPT про `node dist/main.js` **не для этого проекта** — у нас Next.js, команда запуска верная.

**Запасной вариант** (если Nest всё равно падает на apt): в Timeweb тип **Dockerfile**, скопировать `docker/Dockerfile` в корень или указать путь — см. `docker/Dockerfile` (без apt-get).
...
> При деплое через Nest/Node (§1) используются команды сборки и запуска из панели.
...
- **Команда запуска:** **`node start.js`** (из корня репозитория). Запасной вариант: `npm run start`. Не используй `node dist/main.js` (NestJS) и не `node .next/standalone/start.js` — путь к standalone может отличаться на сервере.
...
**Путь проверки состояния:** **`/api/health`** или **`/health`** (оба отдают `{"ok":true}`). Не оставляй пустым, если платформа подставляет `/` — главная может грузиться дольше health-check timeout.

**Зависимости (системные пакеты):** оставь **пустым**. Не добавляй `curl` и `ca-certificates` — иначе Timeweb вызовет `apt install` и сборка упадёт.
