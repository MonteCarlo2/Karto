...
В корне репозитория есть **`Dockerfile`** — multi-stage сборка Next.js standalone **без `apt-get`** (на сборке Timeweb `deb.debian.org` часто недоступен). Образ `node:24-bookworm` уже содержит `ca-certificates`.

**В Timeweb App Platform (минимум правок):**
- **«Зависимости»:** поле **пустое** (не указывайте `curl`, `ca-certificates` и т.п. — иначе платформа снова вызовет `apt install` и сборка упадёт).
- **Тип:** лучше вкладка **Dockerfile**; если оставлен Nest/Node — тоже ок, главное пустые «Зависимости».
- **Команды сборки/запуска** в панели при Dockerfile игнорируются; env и `/api/health` — **не трогать**.

Если деплой **без** Dockerfile (режим Nest/Node) и в логе снова `apt-get` / `curl` — очистите «Зависимости» и пересоберите с очисткой кэша.
