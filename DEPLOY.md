...
В корне репозитория есть **`Dockerfile`** — сборка через него **не вызывает** `apt install curl` и не зависит от DNS `deb.debian.org` в панели.

**В Timeweb App Platform:**
- тип деплоя: **Dockerfile** (или пересоздайте Backend App с репозиторием, где есть Dockerfile в корне);
- **«Зависимости» / system packages:** оставьте **пустым** (ca-certificates уже в образе `node:20-bookworm-slim`);
- **«Путь до директории проекта»:** пусто (корень репозитория);
- команды сборки/запуска в панели при Dockerfile **игнорируются** — старт через `node start.js` из образа.

Если деплой идёт **без** Dockerfile (старый режим фреймворка) и падает на `apt-get` / `curl` — уберите `curl` из «Зависимостей», оставьте только `ca-certificates` или очистите поле и пересоберите.
