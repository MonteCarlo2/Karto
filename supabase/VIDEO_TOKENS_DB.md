...
## Приветственные токены для новых пользователей

При **первом** успешном `GET /api/subscription` (нет активных строк в `user_subscriptions`) создаётся приветственная строка `creative` (3 фото) и вызывается **`add_user_video_tokens(user_id, 100)`**. Константа в коде: `FREE_WELCOME_VIDEO_TOKENS` в `src/lib/subscription.ts`. Нужны рабочие RPC и GRANT, как в миграциях выше.
