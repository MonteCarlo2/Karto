...
**Проверка состояния (Health Check):** в настройках деплоя укажи **«Путь проверки состояния»** = `/api/health`. Иначе платформа может не получать ответ и слать процессу SIGTERM — приложение будет падать сразу после старта.
